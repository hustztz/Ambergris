...
namespace ambergris {
	...
	int SceneGeometryResource::AppendInstance(int mesh, int node)
	{
		int i = 0;
	...
	size_t SceneGeometryResource::GetInstanceSize(int id) const
	{
	...
	int SceneGeometryResource::GetInstance(int id, int inst) const
	{
		if (id >= 0 && id < m_inst_pool.size())
		{
